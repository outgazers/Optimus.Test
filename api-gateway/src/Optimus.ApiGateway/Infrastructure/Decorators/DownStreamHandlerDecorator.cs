...
internal sealed class DownstreamHandlerDecorator : IHandler
{
    ...
    private readonly ILogger<DownstreamHandlerDecorator> _logger;
    ...
    public DownstreamHandlerDecorator(IServiceProvider serviceProvider, IRequestProcessor requestProcessor,
        IPayloadValidator payloadValidator, NtradaOptions options, IHttpClientFactory httpClientFactory,
        ILogger<DownstreamHandlerDecorator> logger)
    {
    ...
    private static HttpContent GetHttpContent(ExecutionData executionData)
    {
        ...
        if (executionData.Context.Request.ContentType.StartsWith("multipart/form-data"))
        {
            var multipartFormDataContent = new MultipartFormDataContent();
            foreach (var f in executionData.Context.Request.Form.Files)
            {
                var stream = new StreamContent(f.OpenReadStream());
                stream.Headers.ContentType = new MediaTypeHeaderValue(f.ContentType);
                multipartFormDataContent.Add(stream, name: f.Name, fileName:f.FileName);
            }
